...
콘솔 창을 손으로 여러 개 띄우는 대신, **가상 클라이언트 N명을 한 번에 만들어** 호스트를 두들깁니다.

```bash
# 터미널 A — 로그를 끄고 호스트 실행 (중요!)
dotnet run -- step3-host quiet

# 터미널 B — 클라 8명이 각각 500개를 5ms 간격으로 전송
dotnet run -- step4-load 127.0.0.1 8 500 5
#                          IP    클라 메시지 간격ms
```

`quiet`가 왜 중요하냐면, 메시지마다 `Console.WriteLine`을 하면 **콘솔 출력이 병목**이 되어 네트워크가 아니라 **화면 출력 속도를 재게 됩니다.** 로그를 켜고 끈 두 결과를 비교해 보면 차이가 확 납니다 — 이것 자체가 좋은 실험입니다.

### 측정 항목

| 항목 | 의미 |
|---|---|
| 전송 처리량 | 클라들이 초당 몇 개를 밀어 넣었나 |
| **브로드캐스트 증폭** | 호스트가 실제로 내보낸 총량 ÷ 받은 총량. 클라 N명이면 **약 N배** |
| **RTT (왕복 지연)** | 내 메시지가 호스트를 거쳐 나에게 돌아오기까지 걸린 시간 |
| 상위 95% | 평균보다 중요. **체감 끊김은 여기서 드러남** |

### 꼭 해볼 실험

1. **간격을 0으로** → `... 8 500 0` : 최대 속도로 쏟아붓기. 처리량 한계와 RTT 폭증을 봅니다.
2. **클라 수를 늘려가며** → 2 → 4 → 8 → 16. RTT가 어디서부터 나빠지는지 확인.
3. **`quiet` 없이 실행** → 로그가 성능을 얼마나 깎는지 비교.
4. **자기 실제 IP로** → `127.0.0.1` 대신 `192.168.x.x`. 랜카드를 거치는 경로와 비교.

### ⚠️ 4단계의 함정 — `Thread.Sleep`은 정확하지 않다

`step4-load ... 5`(5ms 간격)로 돌리면 실제로는 **15.6ms 간격**으로 나갑니다. 윈도우의 기본 타이머 해상도가 15.625ms라 `Sleep(5)`을 불러도 그만큼 자버리기 때문이에요. 그래서 4단계의 처리량은 **호스트 한계가 아니라 `Sleep`이 만든 상한**입니다.

500개를 5ms 간격으로 보냈는데 2.5초가 아니라 7.8초가 걸렸다면 → `7810 ÷ 500 = 15.6ms`. 바로 이 현상입니다.

**정확한 주기가 필요하면 5단계를 쓰세요.**

---

## 5단계 — 게임 조건 테스트 (권장)

실제 게임처럼 **"N명이 초당 K번, T초 동안"** 보내는 방식입니다.

```bash
dotnet run -- step3-host quiet
dotnet run -- step5-game 127.0.0.1 8 20 10 24
#                          IP   인원 Hz 초 바이트
```

위 예시 = **8명이 초당 20번, 10초 동안, 24바이트 위치 패킷**을 보냄.

### 4단계와 무엇이 다른가

| | 4단계 | 5단계 |
|---|---|---|
| 주기 제어 | `Thread.Sleep` (15.6ms로 튐) | **`Stopwatch` 목표 시각 기반** (정확) |
| 설정 방식 | "몇 개 보낼까" | **"몇 Hz로 몇 초"** (게임식) |
| 페이로드 | 짧은 텍스트 | **실제 위치 패킷 크기 흉내** |
| 시작 타이밍 | 전원 동시 | **클라마다 흩뿌림** (실제 프레임 타이밍처럼) |
| 추가 지표 | — | **KB/s 대역폭, Hz 달성률, 프레임 환산** |

### 새로 보는 지표

- **Hz 달성률** — 95% 미만이면 클라가 목표 주기를 못 지킨 것(CPU나 전송이 병목)
- **호스트 업로드 KB/s** — 와이파이가 감당할 수 있는지 판단하는 실질 지표
- **60fps 프레임 환산** — RTT가 몇 프레임에 해당하는지. 1프레임(16.7ms)을 넘기면 눈에 띕니다
...
# 인원을 늘려 O(N²) 확인
dotnet run -- step5-game 127.0.0.1 16 20 10 24
dotnet run -- step5-game 127.0.0.1 32 20 10 24

# 전송 빈도를 높여보기 (30Hz, 60Hz)
dotnet run -- step5-game 127.0.0.1 8 60 10 24

# 패킷을 키워보기 (스케일·색·상태까지 담는 경우)
dotnet run -- step5-game 127.0.0.1 8 20 10 128
```

### 결과 해석의 핵심 — O(N²) 문제

브로드캐스트 방식은 **클라가 N명이면 호스트 부담이 N² 에 비례**합니다. 8명이 각 500개를 보내면 호스트는 **32,000개**를 내보내야 해요(4,000 받고 × 8명에게). 클라를 2배 늘리면 부담은 **4배**가 됩니다.

우리 게임에서 위치 동기화를 초당 20번 한다면, 8명 기준으로 호스트는 **초당 1,280개**를 내보내는 셈입니다. 이 숫자를 실제로 보고 나면 "왜 위치는 UDP로 빼고, 전송 빈도를 줄이고, 관심 영역(가까운 사람에게만) 같은 최적화가 필요한지"가 몸으로 이해됩니다.
