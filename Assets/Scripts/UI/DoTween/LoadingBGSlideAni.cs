...
public class LoadingBGSlideAni : MonoBehaviour
{
    ...
    //   커튼 프리팹의 전환 패널 둘은 꺼진 채로 태어나고, 컨트롤러가 계획을 넣은 <b>뒤에</b>
    //   맞는 쪽만 켠다(SetActive → OnEnable → Play). 그래서 실제로 재생되는 인스턴스는
    //   항상 계획을 갖고 있다.
    //   다섯 인자 모두 선택이므로 아래 호출부에는 null 검사가 남아 있다 —
    //   이 컴포넌트를 커튼 밖에서 단독으로 쓰면 그 경로로 돈다.
