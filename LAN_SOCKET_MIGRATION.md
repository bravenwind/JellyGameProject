...
|---|---|
| 4-1 | 젤리 스폰 — 호스트 권위 |
| 4-2 | 흡수 요청 → 호스트 판정 → 결과 브로드캐스트 (`_claimedJellies` 선착 가드 **그대로 재사용**) |
| 4-3 | 성장·점수·색 반영 |
| 4-4 | 게임 시작/종료 흐름, 결과 씬 전환 |
...
| 항목 | 내용 |
|---|---|
| 5-1 | 배트/대쉬 히트 판정 (호스트 검증) |
| 5-2 | 넉백 — **피격자 소유자에게만** 전송 (위치는 스트림이 전파하므로) |
| 5-3 | 타일 마모·붕괴 + `GameStartTime` 기반 결정론 재생 |
| 5-4 | 생존자 판정 · 종료 처리 |
